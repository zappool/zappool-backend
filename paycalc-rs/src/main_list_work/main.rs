...
fn get_and_print_status_status(conn: &Connection) -> i32 {
    ...
    println!(
        "Workitems: last retrieved/processed: {last_workitem_retrvd} / {last_workitem_time_retrvd}"
    );
...
fn print_workitem_status(conn: &Connection, last_workitem_retrvd: i32) {
    ...
    for wi in newworkitems {
        println!(
            "  {} {} {}{}",
            wi.db_id, wi.tdiff, wi.uname_o, wi.uname_o_wrkr
        );
    }
}
