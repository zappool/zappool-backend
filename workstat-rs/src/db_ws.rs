...
mod tests {
    ...
    fn create_test_db(conn: &Connection) -> Result<(), Box<dyn Error>> {
        db_setup_1(conn)?;
        insert_work_fullname(conn, "lnblabla.worker1", "bc1something.123abc", 131072)?;
        insert_work_fullname(conn, "lnblabla.worker2", "bc1something.321cba", 131072)?;
        insert_work_fullname(conn, "lnblabla.worker1", "bc1something.123abc", 131072)?;
        Ok(())
    }
    ...
    fn test_get_work_after_id() {
        ...
        let all = get_work_after_id(&conn, 0, 1, 0).unwrap();
        assert_eq!(all.len(), 3);

        let after_1 = get_work_after_id(&conn, 1, 1, 0).unwrap();
        assert_eq!(after_1.len(), 2);
        assert_eq!(after_1[0].db_id, 2);
    }
